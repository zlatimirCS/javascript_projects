...
export const signin = async (req, res) => {
  ...
  try {
    ...
    const isPasswordCorrect = await bcrypt.compare(password, existingUser.password); // [2]

    if (!isPasswordCorrect) return res.status(400).json({ message: "Invalid credentials." });

    const token = jwt.sign({ email: existingUser.email, id: existingUser._id }, 'test', { expiresIn: "1h" }); // [3]

    res.status(200).json({ result: existingUser, token }); // [4]
  } catch (error) {
